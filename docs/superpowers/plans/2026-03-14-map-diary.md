...
**Tech Stack:** Next.js (latest, App Router + Turbopack), TypeScript, Tailwind CSS, Kakao Map API, Supabase (PostgreSQL + Storage), Vercel, Jest + React Testing Library

**Prerequisites:** Node.js 20.9 이상 필요 (`node -v`로 확인)
...
Expected: `v20.9.0` 이상. 미만이면 https://nodejs.org 에서 LTS 버전 설치 후 진행.
...
`--yes` 플래그는 공식 권장 기본값을 자동 적용한다:
- ✅ TypeScript
- ✅ ESLint
- ✅ Tailwind CSS
- ✅ App Router
- ✅ Turbopack (v15부터 기본 번들러)
- ✅ `@/*` import alias
- ✅ `src/` 디렉토리 없음
...
- [ ] **Step 7: `.gitignore`에 `.env.local` 포함 여부 확인**
...
Expected: `▲ Next.js X.X.X` + `✓ Starting...` (Turbopack 기본 실행)
...
저장소는 이미 생성되어 있음: `https://github.com/howon-Jeon/my-map-diary`

원격이 설정되지 않은 경우:
```bash
git remote add origin https://github.com/howon-Jeon/my-map-diary.git
```
